...
The GitHub example is manually dispatched and expects labels `self-hosted`, `linux`, and `lynkctl`. The GitLab example uses a manually started web pipeline and a runner tagged `lynkctl`. Run only trusted code on these runners. The examples are templates: generation was tested locally on the public IAR demos, but these integrations have not been executed against a user's firmware runner.
